...
Zeek has a scripting language and can chain multiple events to find an event of interest. We focus on the signatures in this task, and then we will focus on Zeek scripting in the following tasks.  
...
```
Remember, Zeek signatures support regex. Regex ". * " matches any character zero or more times. The rule will match when a "password" phrase is detected in the packet payload. Once the match occurs, Zeek will generate an alert and create additional log files
...
![[Pasted image 20250806212228.png]]
...
![[Pasted image 20250806212925.png]]
...
3. Investigate the **conn.log.  **What is the total number of the sent and received packets from source port 38706?

```shell
cat conn.log | zeek-cut orig_pkts resp_pkts id.orig_p
```
![[Pasted image 20250806225014.png]]
Ans : **20**

4. Create the global rule shown in the task and investigate the **ftp.pcap** file. Investigate the **notice.log.** What is the number of unique events?
...
| --------------------------------------------------------------------------------------------------------------------- |
| /opt/zeek/share/zeek/base/bif<br><br>/opt/zeek/share/zeek/base/bif/plugins<br><br>/opt/zeek/share/zeek/base/protocols |

![[Pasted image 20250808235519.png]]

# Zeek Scripts | Scripts and Signatures


2.  Investigate the ftp.pcap file with **ftp-admin.sig** signature and  **201.zeek** script. Investigate the signatures.log file. What **i**s the number of signature hits?
```shell
...
![[Pasted image 20250809005002.png]]
...
![[Pasted image 20250809005436.png]]

# Zeek Scripts | Frameworks

## Scripts 203 | Load Frameworks  

Zeek's online book [here](https://docs.zeek.org/en/master/frameworks/index.html).
...
![[Pasted image 20250809010924.png]]
...
![[Pasted image 20250809011240.png]]
...
![[Pasted image 20250809011758.png]]
Ans : BroZeek
...
3. ![[Pasted image 20250809012341.png]]
